...
import { GobotStartOptions, GobotConfigOptions } from "./gobots.d"
...
export default class Gobots {
    ...
    public start(options: GobotStartOptions) {
        for (const service of this.gobot.services.enabled) {
            ...
            switch(service) {
                ...
                case "discord":
                    ...
                    if(options.verbose)
                        console.info("Listening on Discord...")
                    ...
                    discordClient.on("message", message => {
                        ...
                        for(const action of this.gobot.actions) {
                            if (action.question && message.content === action.question)
                                message.reply(action.response)
                        }
